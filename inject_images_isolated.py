...
# Map buoi prefix (e.g. '04') to a dictionary of images for that buoi
available_images_by_buoi = {}

for subdir in os.listdir(fig_dir):
    if not subdir.startswith('Buoi_'): continue
    # Extract the buoi number, e.g. '04' from 'Buoi_04A'
    buoi_num = re.search(r'Buoi_(\d+)', subdir)
    if not buoi_num: continue
    buoi_key = buoi_num.group(1)
    
    if buoi_key not in available_images_by_buoi:
        available_images_by_buoi[buoi_key] = {}
    ...
    if os.path.isdir(subdir_path):
        for img in os.listdir(subdir_path):
            ...
            if id_match:
                ...
                available_images_by_buoi[buoi_key][identifier] = {
                    'filename': img,
...
for md_filename in os.listdir(docs_dir):
    ...
    # Extract buoi number from md filename, e.g. '04' from 'buoi_04.md'
    buoi_match = re.search(r'buoi_(\d+)', md_filename)
    if not buoi_match: continue
    buoi_key = buoi_match.group(1)
    
    # If we have no images for this buoi, skip
    available_images = available_images_by_buoi.get(buoi_key, {})
