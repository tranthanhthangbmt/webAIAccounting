...
img_dir = r"d:\DongAUniversity\TÀI LIỆU DẠY HỌC_2024-2025\Môn TTNT cho kế toán_2026\webAIAccounting\TaiLieu\textbookForPractice\Figures\Ch_03"
all_images = [f for f in os.listdir(img_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]

def get_images(prefix):
    # e.g., prefix = "ILLUSTRATION 3."
    imgs = [img for img in all_images if img.startswith(prefix)]
    # Sort them by number
    def sort_key(x):
        # Extract the first number after the prefix
        m = re.search(r'(\d+)', x[len(prefix):])
        if m:
            return float(m.group(1))
        return 0
    
    # Actually, some are like "ILLUSTRATION 3.1.png", "ILLUSTRATION 3.10.png"
    # To sort correctly:
...
illus = get_images("ILLUSTRATION 3.")
bes = get_images("BE 3.")
exs = get_images("EX 3.")
pacs = get_images("PAC 3.")

# Split illus into 6 chunks
chunk_size = len(illus) // 6
chunks = [illus[i:i + chunk_size] for i in range(0, len(illus), chunk_size)]
# Adjust if there's a remainder
if len(chunks) > 6:
    chunks[5].extend(chunks[6])
    chunks = chunks[:6]

# Read original slide_data_ch03.py
with open("slide_data_ch03.py", "r", encoding="utf-8") as f:
    content = f.read()

# We will just write a completely new file to ensure it's correct.
# Wait, it's easier to just rebuild slide_data_ch03.py from scratch here.
new_code = r'''chapter_title = "Động lực và Mục tiêu Phân tích Dữ liệu"
chapter_subtitle = "Motivations and Objectives for Data Analysis"
...
\end{itemize}"""
    ...
    {
        "type": "normal",
        "title": "3.1 Động lực định hình câu hỏi phân tích như thế nào?",
        "content": r"""\begin{itemize}
...
\end{itemize}"""
    ...
    {
        "type": "normal",
        "title": "Mục tiêu Rõ ràng (Clear Objectives)",
        "content": r"""\begin{itemize}
...
\end{itemize}"""
    ...
    {
        "type": "normal",
        "title": "Xác định Mục tiêu (Determine the Objective)",
        "content": r"""\begin{itemize}
...
\end{itemize}"""
    ...
    {
        "type": "normal",
        "title": "Kết nối Động lực với Mục tiêu",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "3.2 Làm thế nào để phát triển Câu hỏi Mô tả?",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Đặc điểm của Câu hỏi Mô tả",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Phân loại Câu hỏi Mô tả trong Kế toán",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Ví dụ Phân tích Mô tả (Kế toán Tài chính)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Dữ liệu bán hàng & Trực quan hóa",
        "content": r"""\begin{itemize}
...
new_code += add_images(["Apply It 3.2.png"])

new_code += r'''
slides.append({
        "type": "normal",
        "title": "3.3 Làm thế nào để phát triển Câu hỏi Chẩn đoán?",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Đặc điểm của Câu hỏi Chẩn đoán",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Đào sâu vào Dữ liệu (Drill-down)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Ví dụ Phân tích Chẩn đoán (Kiểm toán)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Minh họa dữ liệu và trực quan hóa cảnh báo",
        "content": r"""\begin{itemize}
...
new_code += add_images(["Apply It 3.3.png"])

new_code += r'''
slides.append({
        "type": "normal",
        "title": "3.4 Làm thế nào để phát triển Câu hỏi Dự đoán?",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Đặc điểm của Câu hỏi Dự đoán",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Mô hình Phân loại và Dự báo",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Ví dụ Phân tích Dự đoán (Kế toán Quản trị)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Đánh giá xu hướng chuỗi thời gian",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Trực quan hóa dữ liệu dự đoán",
        "content": r"""\begin{itemize}
...
new_code += add_images(["Apply It 3.4.png"])

new_code += r'''
slides.append({
        "type": "normal",
        "title": "3.5 Làm thế nào để phát triển Câu hỏi Đề xuất?",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Đặc điểm của Câu hỏi Đề xuất",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Phân tích What-If và Tối ưu hóa",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Ví dụ Phân tích Đề xuất (Kế toán Quản trị)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Giải quyết bài toán ràng buộc nguồn lực",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Trực quan hóa kết quả phân tích Đề xuất",
        "content": r"""\begin{itemize}
...
'''
new_code += add_images(chunks[4])

new_code += r'''
slides.append({
        "type": "normal",
        "title": "3.6 Ứng dụng trong Thực tiễn Nghề nghiệp",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Kế toán Tài chính (Financial Accounting)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Kế toán Quản trị (Managerial Accounting)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Kế toán Thuế (Tax Accounting)",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Ghép nối Động lực với các Lĩnh vực Chuyên môn",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Tóm tắt Mục tiêu Học tập 1",
        "content": r"""\begin{itemize}
...
slides.append({
        "type": "normal",
        "title": "Tóm tắt Mục tiêu Học tập 2",
        "content": r"""\begin{itemize}
...
'''
# Group BEs 2 by 2
for i in range(0, len(bes), 2):
    if i+1 < len(bes):
        img1 = bes[i]
        img2 = bes[i+1]
        title = f"{os.path.splitext(img1)[0]} & {os.path.splitext(img2)[0]}"
        new_code += f'''
slides.append({{
    "type": "double_image",
    "title": "{title}",
...
'''
        ...
        new_code += add_images([pacs[i]])

# Any remaining images that are NOT Illustration, BE, EX, PAC
other_imgs = [img for img in all_images if not any(img.startswith(prefix) for prefix in ["ILLUSTRATION 3.", "BE 3.", "EX 3.", "PAC 3.", "Apply It"])]
if len(other_imgs) > 0:
    new_code += r'''
slides.append({
    "type": "normal",
...
with open(r"d:\DongAUniversity\TÀI LIỆU DẠY HỌC_2024-2025\Môn TTNT cho kế toán_2026\webAIAccounting\TaiLieu\slidePractice\slide_data_ch03.py", "w", encoding="utf-8") as f:
    f.write(new_code)
