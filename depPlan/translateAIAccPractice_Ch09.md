# Kế hoạch dịch thuật chi tiết: Chương 9 - Communicating Data Analysis Results

## 1. Mục tiêu và Tài liệu nguồn
- **Nguồn:** `TaiLieu/textbookForPractice/Ch_09_Interpreting Data Analysis Results.pdf`
...
- **Đầu ra:** File Markdown `docs/practice_ch09.md` để đưa lên trang web (Docsify) và được liên kết trong menu `_sidebar.md`.

## 2. Phương pháp Phân chia (Chunking Strategy)
Tài liệu (62 trang) sẽ được chia thành nhiều phần (chunks) dựa trên cấu trúc các mục tiêu học tập (LO) và các phần bài tập:
- **Chunk 1:** Phần mở đầu & LO 9.1
- **Chunk 2:** LO 9.2
- **Chunk 3:** LO 9.3
- **Chunk 4:** LO 9.4
- **Chunk 5:** LO 9.5
...
- **Chunk 9:** Problems (PR) & Professional Application Cases (PAC) (Nếu có). Yêu cầu **dịch chi tiết toàn bộ** và **chèn đầy đủ các hình ảnh** liên quan.
...
- **Cú pháp:** `![Tên_Ảnh](../TaiLieu/textbookForPractice/Figures/Ch_09/Tên_Ảnh.png)`
- Nguồn hình ảnh: Lấy từ thư mục `TaiLieu/textbookForPractice/Figures/Ch_09` (đã có sẵn các file như `ILLUSTRATION 9.1.png`, `EX 9.10.png`, `Apply It 9.1.png`...).
...
1. **Extract Text:** Trích xuất text thô từ PDF, chia vào các file `scratch/ch09_chunk_*.txt`.
2. **Translate (Dịch):** Đẩy từng Chunk vào thư viện dịch thuật để dịch sang tiếng Việt và lưu kết quả vào `scratch/ch09_tr_chunk_*.md`.
3. **Update Images:** Chạy script để tìm và chèn link ảnh từ thư mục `Figures/Ch_09` vào đúng các vị trí đánh dấu trong file Markdown.
4. **Assemble (Lắp ráp):** Hợp nhất các bản dịch lại thành file `docs/practice_ch09.md` thông qua script Python, tự động:
   - Xóa các artifact thừa (số trang, header rác của PDF).
   - Xử lý các tag in đậm cho các mục lục `LO 9.x` và `MINH HỌA/EX/BE/Apply It`.
   - Giữ nguyên đánh số danh sách (`1. `, `2. `).
5. **Update UI:** Cập nhật file `_sidebar.md` để gắn link hiển thị bài **Thực hành 9** lên thanh điều hướng của website.
