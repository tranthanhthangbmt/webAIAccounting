...
def translate_file(input_path):
    output_path = input_path.replace('.txt', '.md').replace('ch08_chunk_', 'ch08_tr_chunk_')
...
if __name__ == "__main__":
    chunks = glob.glob(r"scratch\ch08_chunk_*.txt")
    chunks.sort(key=lambda x: int(re.search(r'ch08_chunk_(\d+)', x).group(1)))
