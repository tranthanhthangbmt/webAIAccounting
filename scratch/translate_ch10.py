...
KEYWORDS = [
    ...
    "Tableau", "Alteryx", "Information Modeling", "Data", "Recent Data and Analyses Developments in Accounting",
    "Developments"
]

def translate_file(input_path):
    output_path = input_path.replace('.txt', '.md').replace('ch10_chunk_', 'ch10_tr_chunk_')
...
if __name__ == "__main__":
    chunks = glob.glob(r"scratch\ch10_chunk_*.txt")
    chunks.sort(key=lambda x: int(re.search(r'ch10_chunk_(\d+)', x).group(1)))
