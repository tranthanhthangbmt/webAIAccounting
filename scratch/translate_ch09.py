...
KEYWORDS = [
    "Accounting", "Analytics", "SQL", "Excel", "Data Exploration", "Data Preparation",
    "Tableau", "Alteryx", "Information Modeling", "Data", "Interpreting Data Analysis Results",
    "Communicating Data Analysis Results", "Communicating"
]

def translate_file(input_path):
    output_path = input_path.replace('.txt', '.md').replace('ch09_chunk_', 'ch09_tr_chunk_')
...
if __name__ == "__main__":
    chunks = glob.glob(r"scratch\ch09_chunk_*.txt")
    chunks.sort(key=lambda x: int(re.search(r'ch09_chunk_(\d+)', x).group(1)))
