import os
import glob
import re
from deep_translator import GoogleTranslator

# Keywords to preserve in parentheses
KEYWORDS = [
    "Accounting", "Analytics", "SQL", "Excel", "Data Exploration", "Data Preparation",
    "Tableau", "Alteryx", "Information Modeling", "Data"
]

def translate_file(input_path):
    output_path = input_path.replace('.txt', '.md').replace('ch07_chunk_', 'ch07_tr_chunk_')
    print(f"Translating {input_path} -> {output_path}")
    
    with open(input_path, 'r', encoding='utf-8') as f:
        text = f.read()
        
    paragraphs = text.split("\n\n")
    translator = GoogleTranslator(source='en', target='vi')
    translated_paragraphs = []
    
    for p in paragraphs:
        p = p.strip()
        if not p:
            translated_paragraphs.append("")
            continue
            
        if p.startswith("--- PAGE"):
            continue
            
        if len(p) > 4900:
            sentences = p.split('. ')
            temp_p = ""
            for s in sentences:
                if s.strip():
                    try:
                        trans_s = translator.translate(s)
                        temp_p += trans_s + ". "
                    except Exception as e:
                        print("Error translating sentence:", e)
                        temp_p += s + ". "
            translated_paragraphs.append(temp_p)
        else:
            try:
                trans = translator.translate(p)
                translated_paragraphs.append(trans)
            except Exception as e:
                print(f"Error translating paragraph of length {len(p)}: {e}")
                translated_paragraphs.append(p)
                
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("\n\n".join(translated_paragraphs))
    print(f"Saved {output_path}")

if __name__ == "__main__":
    chunks = glob.glob(r"scratch\ch07_chunk_*.txt")
    chunks.sort(key=lambda x: int(re.search(r'ch07_chunk_(\d+)', x).group(1)))

    for chunk in chunks:
        translate_file(chunk)

    print("All chunks translated!")
