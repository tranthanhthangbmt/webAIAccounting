...
def translate_file(input_path):
    output_path = input_path.replace('.txt', '.md').replace('ch06_chunk_', 'ch06_tr_chunk_')
...
if __name__ == "__main__":
    chunks = glob.glob(r"scratch\ch06_chunk_*.txt")
    chunks.sort(key=lambda x: int(re.search(r'ch06_chunk_(\d+)', x).group(1)))
